...
var Reporter_html = function(options) {
    ...
    function formatOutput(output) {
        ...
        output.suites.forEach(function(suite) {
            if(suite.status === "passed"){
                html += '<tr class="suite passed">';
            }else{
                html += '<tr class="suite failed">';
            }
            html += '<td colspan="4">Suite: ';
            html += suite.description + ' -- ' + suite.status + '</h3></td>';
            suite.specs.forEach(function(spec){

                if(spec.status === "passed") {
                    html += '<tr class="spec passed">';
                }else {
                    html += '<tr class="spec failed">';
                }
                html += '<td>' + spec.description +'</td>';
                html += '<td>' + spec.status + '</td>';
                html += '<td><a href="' + 'screenshots/' + spec.screenshot + '" class="screenshot">';
                        html += '<img src="'+ 'screenshots/' + spec.screenshot + '"/>';
                        html += '</a></td><td>';
                if (spec.failedExpectations) {
                    spec.failedExpectations.forEach(function (fe) {
                        html += 'message: ' + fe.message;
                    });
                }
                html += '</td></tr>';
            })
    ...
    function initOutputFile(outputFile) {
        ...
        var header = '<!DOCTYPE html><html><head lang=en><meta charset=UTF-8><title></title><style>table, table>tbody>tr>td {border: 1px solid #ddd; padding: 5px;} tr.suite.passed{background-color: #67b167;} tr.suite.failed{background-color: #f77878;} tr.spec.passed{color: green;} tr.spec.failed{color: red;} img{border: 3px solid #ddd; width:100px; height: 100px;}</style></head>';
